...
v0.1 Foundation зібрано приблизно на 95%. Тришарова модель активна як цикл, multi-pass compression реалізований (5 LLM-проходів + consolidator), Archive → Core bridge працює через LLM-output з gating-правилами. Telegram host уже має розширений набір debug/core/archive-команд і локальний кеш ключів для швидкого тестування. Важлива поправка: event-count auto-sleep (`after_events`, default 50) є dev/test accelerator, а не продуктовий режим. Продуктові режими sleep — досягнення token/context budget і scheduled idle sleep о 04:00. Бракує: довести ці production sleep triggers, emotional path для Core promotion, демотацію як завершений roadmap-пункт, формальний сценарний fixture і code-only GitHub push. Після цих пунктів v0.1 закривається і починається v0.2 (reflection + embeddings).
...
- [x] Dev/test auto-sleep при N незаархівованих подіях (default 50) із deduplication pending sleep task. Це тестовий прискорювач і emergency guard, не продуктовий sleep trigger.
- [ ] Product sleep trigger від token/context budget pressure: коли active session/current memory наближається до budget, система стискає старшу частину сесії і лишає свіжий хвіст.
...
- [ ] Scheduled idle sleep у Telegram host: якщо є незаархівовані події, bot запускає sleep о 04:00 локального часу або через конфігурований nightly schedule. Це продуктовий шлях фонового sleep, на відміну від dev/test `after_events`.
...
- [ ] **Сценарний тест базової памʼяті**: end-to-end fixture, що проганяє `ingest` → sleep → `recall` → `core_context_package` і доводить повний цикл. Для швидкості тест може використовувати dev/test `after_events`, але acceptance не має залежати від "50+ івентів" як продуктової межі.
- [ ] **Сценарний тест production sleep triggers**: окремо перевірити budget-pressure sleep і scheduled idle sleep. Event-count trigger не зараховується як production sleep trigger.
...
- [ ] `forget_review_pass`: окремий LLM-agent періодично отримує старі `compact_memory` тези з freshness/weight/recall_count і рекомендує, які спогади природно позначити як forgotten. Engine не видаляє full archive, а переносить/позначає зі збереженням audit trail.
