...
v0.1 Foundation зібрано приблизно на 95%. Тришарова модель активна як цикл, multi-pass compression реалізований (5 LLM-проходів + consolidator), rolling / partial sleep працює, Archive → Core bridge переносить `personal_signals` у Core через gating-правила без regex extraction. Живі Telegram-тести підтвердили sleep, archive tracks, Core backfill і Core management. Бракує: формальний сценарний fixture для end-to-end перевірки циклу, оновлення `github-code` branch і перший code-only push. Після цього v0.1 можна закривати і переходити до v0.2: reflection, candidate beliefs, emotional promotion path, embeddings і кращий merge/dedup Core-фактів.
...
- [x] Архітектура `docs/architecture.md` v0.1 із multi-pass sleep, rolling / partial sleep і каналами Core.
- [x] Контракти `docs/contracts.md` із усіма schema-формами.
- [x] `HISTORY.md` як document of trust за MemPalace-template.
- [x] `docs/local-development.md` із інсталяцією і командами.
...
- [x] Partial sleep при ліміті контексту: стискати старшу частину unarchived window, лишати свіжий active tail у session. — 2026-05-18, `87964b5`, `SleepStage1Config.active_tail_ratio`, `partial_sleep_min_events`.
- [x] Публічний шлях демотації Core-факту через `patch_core_fact { status: Deprecated }`. — 2026-05-19, `crates/memory_engine/src/engine.rs`, `hosts/telegram_gemini_bot/bot.py`.
...
- [x] Канал 3 — Archive → Core bridge через `personal_signals` із gating: `confidence >= 0.85`, mapped category, user_source guard, near-duplicate detection. — 2026-05-20, `928a7b2`, `178bff4`.
...
- [x] Чат-промпт Telegram host через `prompts/telegram_chat_system.md`: використовує context package як source of truth і не вигадує памʼять поза ним.
- [x] Команда `/core_forget <id>` через `patch_core_fact` зі статусом `Deprecated`. — 2026-05-19, `hosts/telegram_gemini_bot/bot.py`.
...
- [x] Guard для slash-команд: `/archive` без id показує usage, невідомі `/...` не йдуть у Gemini і не засмічують session memory. — 2026-05-20, `4d1244a`.
- [x] Live-тест на сесіях із кішкою Іржею, космосом і profile facts. Маркери успіху: `archive_1779305322363339900_25` і `archive_1779305450448607200_32` мають `emotional_markers` і `personal_signals`; Core backfill додав факти про користувача та Іржу; pending sleep tasks не лишились. — 2026-05-20, `DEVLOG.md` Запис 47.
...
- [~] **Сценарний тест базової памʼяті**: live-перевірка на Telegram пройдена, але формальний fixture ще не написаний. Має проганяти 50+ івентів через `ingest` → auto-sleep → `recall` → `core_context_package`.
...
- [x] `SleepStage1Config.active_tail_ratio: f64` (default 0.30) і `partial_sleep_min_events`. Sleep стискає старшу частину великого unarchived window, свіжий хвіст залишає активним. — 2026-05-18, `87964b5`.
- [~] Live-тест: довга сесія й manual sleep уже проходили, але потрібен окремий формальний сценарій, який доводить, що bot не втрачає останні кілька повідомлень після auto-sleep.
