...
- [ ] Schema `MemoryUnit`: атомарний спогад із короткою тезою, `event -> conclusion`, source ids, weight, status, fidelity status і короткою локальною label-формою для prompt (`m1`, `m2`, `m3`), щоб не тягнути довгі технічні ID у LLM-контекст.
- [ ] `memory_unit_pass`: LLM-agent розбиває sleep/archive material на стільки змістових одиниць, скільки реально є в розмові. Жодних fixed quotas на кшталт "5-7 тез".
- [ ] `compact_memory` стає prompt-проекцією memory units, а не другим LLM-підсумком. У prompt іде `core_memory` + `long_memory` + `short_memory` + `current_user_message`, з явними межами.
- [ ] Prompt geometry рендериться XML-подібними тегами або рівноцінними чіткими секціями; `telegram_chat_system.md` пояснює різницю між `long_memory`, `short_memory` і `current_user_message`.
...
- [ ] MemoryUnit schema/storage.
- [ ] `memory_unit_pass` замість окремого `compact_memory_pass`.
- [ ] `compact_memory` як projection із units.
- [ ] Prompt geometry з тегами/секціями.
