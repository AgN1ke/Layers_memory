...
Після v0.2 іде короткий audit-cleanup перед v0.3: A1/A2/A3/A4 step 1/A4 step 3/B3/A5/A7/B4 уже закриті. Відкритим перед v0.3 лишається головний технічний борг із `docs/audit-2026-06-10.md`: розріз великого `engine.rs` на модулі (B2). Journal/recovery scope (A3) закрито як рішення: journal лишається storage primitive для майбутніх migration/recovery-heavy операцій, runtime sleep спирається на durable `SleepRun` checkpoints. v0.3-меню після cleanup: Godot-адаптер як другий хост, reviewer-pass, opt-in vector storage, unit-level recall counters.
