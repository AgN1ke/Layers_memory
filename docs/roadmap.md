...
Після v0.2 іде короткий audit-cleanup перед v0.3: A1/A2/A4 step 1/A4 step 3/B3/A5/A7/B4 уже закриті. Відкритими перед v0.3 лишаються масштабувальні й технічні борги з `docs/audit-2026-06-10.md`: розріз великого `engine.rs` на модулі (B2) і рішення щодо journal/recovery scope (A3, якщо власник обирає підключати журнал зараз). v0.3-меню після cleanup: Godot-адаптер як другий хост, reviewer-pass, opt-in vector storage, unit-level recall counters.
