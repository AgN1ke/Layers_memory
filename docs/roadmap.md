...
- [ ] Token-budget allocator для `core_context_package`: максимум 11k токенів памʼяті в prompt, із розподілом 7k поточна памʼять / 3k стиснута памʼять / 1k Core. Стиснення має зберігати сенси, емоційні маркери і personal signals, а не просто обрізати текст.
...
- [x] Канал 3 — Archive → Core bridge через `personal_signals` із gating: `confidence >= 0.85`, mapped category, user_source guard, near-duplicate detection.
...
- [ ] Live-тест на сесії "ім'я → літаки → Маріана → кішка → Європа Юпітера → що ти про мене знаєш?". Маркери успіху: кішка в `emotional_markers`, кішка в `personal_signals`, кішка в Core через bridge, bot згадує її на питання "що знаєш про мене".
