...
Після v0.2 іде короткий audit-cleanup перед v0.3: A1/A2/A4 step 1/B3/A5 уже закриті; цей документ закриває A7 doc sync. Відкритими перед v0.3 лишаються масштабувальні й технічні борги з `docs/audit-2026-06-10.md`: кеш `archived_event_ids` у metadata (A4 step 3), розріз великого `engine.rs` на модулі (B2), явна фіксація однопроцесної моделі (B4) і рішення щодо journal/recovery scope (A3, якщо власник обирає підключати журнал зараз). v0.3-меню після cleanup: Godot-адаптер як другий хост, reviewer-pass, opt-in vector storage, unit-level recall counters.
