...
- [ ] Фаза B за ТЗ: `recall_deep` (min_sim-чесність, top_k, buffered reinforcement), локальний fastembed embedder у Telegram host, `/vectors*` і `/recall_deep` команди, conformance `--host direct-vectors` — реалізовано. Залишається перед Phase C: автоматичне Gemini function-calling підключення `recall_distant_memory` і live-калібрування `min_sim` на живих даних.
