...
- `session_recent`: short-term turn context, follow-ups, and pronouns.
- `session_trace`: what has been discussed in this chat.
- `archive_relevant`: older committed memories returned by recall.
- `domain_state`: current host state and current message metadata.

For questions about what has been discussed, prefer `session_trace` first, then `archive_relevant`.
