...
- оновлено `docs/architecture.md`, `docs/contracts.md` і `HISTORY.md`.
...
## Запис 93 — 2026-06-10 — Повний аудит після v0.2: знайдені проблеми зафіксовано в docs/audit-2026-06-10.md
...
- Створено `docs/audit-2026-06-10.md`: 7 проблем (A1–A7) і 5 пропозицій (B1–B5) з пріоритетами і порядком робіт.
...
- У `docs/audit-2026-06-10.md` пункт A1 позначено як закритий.
...
**Що робили:**
- Додали `docs/v0.3-acceptance.md` з матрицею direct / Telegram bot / Telegram userbot / Godot.
- Додали перший deterministic direct driver: `tests/host_conformance/host_conformance.py --host direct`.
- Оновили v0.3 секцію `docs/roadmap.md`: direct driver є baseline, Telegram/Godot driver-и мають наслідувати той самий сценарій.
...
## Запис 117 — 2026-06-12 — Незалежний розбір docs/research/multi-speaker-geometry-2026-06-12.md (Claude)

**Проблематика:**
Власник із Claude зафіксували дослідження про геометрію повідомлень для мультиспікерного групового чату (`docs/research/multi-speaker-geometry-2026-06-12.md`). Перед перенесенням у roadmap потрібен незалежний розбір документа проти фактичного коду: чи точні його твердження про поточний стан, чи є прогалини в рішенні, і які нюанси реалізації документ не побачив.
...
3. **Gating Фази 1 — тільки в ядрі, не в хості.** Документ пише "кілька рядків gating у хості/ядрі" — це треба зафіксувати жорстко: "що потрапляє в Core" стоїть у списку заборон хоста в `docs/v0.3-acceptance.md`. Ядро саме бачить speaker-різноманіття серед source events архіву і саме вимикає auto-bridge (зі збільшенням `skipped` у `CoreSignalSummary`).
...
**Задум:**
Проаналізувати проти коду, де в системі вже є час, де його немає, і спроєктувати рішення в межах інваріантів (ядро без мережі, файли — правда, не вигадувати наперед). Результат зафіксовано в `docs/research/memory-time-perception-2026-07-02.md` за тим самим форматом, що й geometry-документ, для перенесення в roadmap.
