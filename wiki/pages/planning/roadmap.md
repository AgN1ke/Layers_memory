...
- [ ] Фаза B за ТЗ: `recall_deep` (min_sim-чесність, top_k, buffered reinforcement), локальний fastembed embedder у Telegram host, `/vectors*` і `/recall_deep` команди, conformance `--host direct-vectors` — реалізовано. Live-калібрування на owner scope виконано для MiniLM (`min_sim=0.30`, `vivid_sim=0.55`). Залишається перед Phase C: автоматичне Gemini function-calling підключення `recall_distant_memory` у звичайний чат.
